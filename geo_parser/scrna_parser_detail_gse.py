...
model_disease = pd.read_excel("geo_parser/dc_disease.xls")
model_cellpops = pd.read_excel("geo_parser/dc_cellpops.xls")
model_celltypes = pd.read_excel("geo_parser/dc_celltypes.xls")
model_celllines = pd.read_excel("geo_parser/dc_lines.xls")
model_papers    = pd.read_excel("geo_parser/dc_papers.xls")#,engine='xlrd')
model_tissues   = pd.read_excel("geo_parser/dc_tissues.xls")
model_journal   = pd.read_excel("geo_parser/dc_journals.xls")
...
def parse_field_df(description, df):
    found_terms = []
    for i, x in enumerate(list(df['name'])):
        if string_found(str(x),description):
            found_terms.append(i)
    return list(df['name'].loc[found_terms])
...
def _parse_a_field(description_dict, a_field, DCmodel, model_df, max_create_length=100, new=False):
    ...
    if len(description_dict.get(a_field, "")) > 0:
        if DCmodel in [models.CellTypes]: #remove '-' when match cell types, like T-cells > T cells
            ...
            reverse=True)'''
    result_searched_by_name = parse_field_df(description_dict[a_field], model_df)  
        
    if result_searched_by_name: 
    #and len(result_searched_by_name[0].name.strip()) > 0    
        
        # 
        # if(len(result_searched_by_name) > 0):
        #     print("parsed result is")
        #     print(description_dict[a_field])
        #     print(result_searched_by_name[0])
    ...
    if new and (len(description_dict.get(a_field, "")) > 0) and (len(description_dict.get(a_field, "")) <= max_create_length):
        ...
        if DCmodel in [models.syn_lines, models.CellTypes, models.TissueTypes]: # these three will check wheather they appare in each other
            return description_dict[a_field]
        else:
...
def _parse_fields(description_dict, strict_fields, greedy_fields, DCmodel, model_df, greedy_length=100):
    for sf in strict_fields:
        ret = _parse_a_field(description_dict, sf, DCmodel, model_df, greedy_length, new=False)
        if ret:
            print("here45")
            print(ret)
            return ret
    ...
    for gf in greedy_fields:
        ret = _parse_a_field(description_dict, gf, DCmodel, model_df, greedy_length, new=True)
        if ret:
            print("here45")
            print(ret)
            return ret
    ...
    ret = _parse_a_field(characteristics, 'characteristics', DCmodel, model_df, greedy_length, new = False)
    if ret:
        print("here45")
        print(ret)
        return ret

    #if DCmodel in [model_celllines]: # search cell line by pubic database information
...
def parseCellType(description_dict):
    return _parse_fields(description_dict,
                         ...
                         ['cell type'],
                         models.CellTypes, model_celltypes)


def parseCellPop(description_dict):
    return _parse_fields(description_dict, ['cell', 'source name', 'cell description', 'title', 'cell type', 'cell lineage'], [], models.CellPops, model_cellpops)

def parseCellLine(description_dict):
    return _parse_fields(description_dict, ['cell', 'source name', 'cell description', 'title', 'cell line', 'cell lineage'], [], models.syn_lines, model_celllines)
...
def parseTissue(description_dict):
    return _parse_fields(description_dict,
                         ...
                         ['tissue', 'tissue type'],
                         models.TissueTypes, model_tissues)
...
def parseDisease(description_dict):
    return _parse_fields(description_dict,
                         ...
                         ['disease'],
                         models.DiseaseStates, model_disease)
...
# description_dict, a_field, DCmodel, model_df, max_create_length=100, new=False
def search_between_table(description_dict, strict_fields, serious_fields, DCmodel, model_df):
    for sf in strict_fields:
        sea = _parse_a_field(description_dict, sf, DCmodel, model_df, 100, new=False)
        if sea:
            return sea
    tmp = {}
    ...
    characteristics = {'characteristics':' '.join(list(tmp.values()))}
    sea = _parse_a_field(characteristics, 'characteristics', DCmodel, model_df, 100, new = False)
    if sea:
        return sea
            ...
            if content:
                ...
                if proCell:
                    ...
                    if Cell.lower() not in ['cell', 'cancer', 'tumor', 'clone', 'cell line', 'cells', 'human cell line', 'mouse cell line', 'cellline', 'celllines']:
                        if (not _parse_a_field({'cellType':Cell}, 'cellType', models.CellPops, model_cellpops, 100, new = False)) and (not _parse_a_field({'cellType':Cell}, 'cellType', models.syn_lines, model_celllines, 100, new = False)) : # in case metamap result in cell pop
    ...
    if serious_fields:
        for sf in serious_fields:
            ...
            if tmp and (DCmodel == models.CellTypes) and (not _parse_a_field({'cell':tmp}, 'cell', models.syn_lines, model_celllines, 100, new = False)) and (not _parse_a_field({'cell':tmp}, 'cell', models.TissueTypes, model_tissues, 100, new = False)) and (not _parse_a_field({'cell':tmp}, 'cell', models.CellPops, model_cellpops, 100, new = False)):
                return tmp
            if tmp and (DCmodel == models.syn_lines) and (not _parse_a_field({'cell':tmp}, 'cell', models.CellTypes, model_celltypes, 100, new = False)):
                return tmp
            if tmp and (DCmodel == models.TissueTypes) and (not _parse_a_field({'cell':tmp}, 'cell', models.CellTypes, model_celltypes, 100, new = False)) and (not _parse_a_field({'cell':tmp}, 'cell', models.syn_lines, model.celllines, 100, new = False)):
                return tmp
            if tmp and (DCmodel not in [models.CellTypes, models.syn_lines, models.TissueTypes]):
                return tmp
    return None
...
def parseAndsearch(description_dict, field):
    tmp_sea_cellLine = search_between_table(description_dict,
                         field, ['cell line'],
                         models.syn_lines, model_celllines)
    ...
    tmp_sea_cellpop = search_between_table(description_dict,
                         field, [],
                         models.CellPops, model_cellpops)
...
def update_one_sample(gseid, ddir='geo', parse_fields=['other_ids', 'paper', 'name', 'species', 'description', 'factor',
    ...
    fields = ['Series/Title', 'Series/Summary', 'Series/Type',
            'Series/Overall-Design', 'Series/Status/Release-Date', 'Series/Status/Last-Update-Date',
             'Sample/Accession', 'Series/Pubmed-ID', 'Sample/Channel/Characteristics', 'Sample/Relation']
    sample_path = os.path.join(ddir+'/'+gseid[:7]+'/'+gseid+'.xml')
    xmlContent = getGEOSamples_byType_gse._getFieldXML(sample_path, ddir,fields = fields)
    ...
    if 'other_ids' in parse_fields and 'Sample/Relation' in xmlContent:
        print('+++ srx')
        try:
            #pmid = 1
            #print(pmid)
            srx = xmlContent['Sample/Relation']
            #print(paper)
    ...
    #testchar = xmlContent['Sample/Channel/Characteristics'].attrib['age']
    #print("TESTTEST:", testchar)
    ...
    if 'cell type' in parse_fields:
        ...
        searchCellType = parseAndsearch(description_dict,
                                        ['cell type', 'cell lineage', 'cell', 'cell line', 'source name', 'cell description', 'title'])
        if searchCellType['cellType'] and (
                str(searchCellType['cellType']).upper() not in [str(searchCellType['cellLine']).upper(),
    ...
    if 'cell line' in parse_fields:
        cell_line = parseCellLine(description_dict)
        if searchCellType['cellLine']:
            print('+++ cell line')
            cell_line = searchCellType['cellLine']
    ...
    res = [gseid, str(species), str(pmid), str(paper), str(name), str(tmp_celltype),
        ...
        str(srx), 
        str(cell_line), str(accession), str(xmlContent)]
    time.sleep(3)
    return res
